...
class App extends Component {
  ...
  deleteUser = id => {
    ...
    const filteredUserData = historyDetailsList.filter(each => each.id !== id)
    this.setState({
      historyDetailsList: filteredUserData,
    })
  }
  ...
  render() {
    ...
    return (
      <div className="background-class">
        <div className="blue-search-class">
          <img
        ...
        {searchResults.length !== 0 ? (
          ...
          <div className="center-container">
            <p className="paragraph-no-class">There is no history to show</p>
          </div>
        )}
      </div>
    )
